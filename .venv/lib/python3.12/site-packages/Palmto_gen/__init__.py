...
class DisplayTrajs():
    ...
    def plot_map(self, trajs):
        ...
        # TRAVA DE SEGURANÇA 1: Verifica se a lista principal está vazia
        if not trajs or (len(trajs) > 0 and len(trajs[0]) == 0):
            print("Aviso no plot_map: Nenhuma trajetória recebida para plotar. Gerando mapa em branco.")
            return folium.Map(location=[0, 0], zoom_start=2)

        # Encontra a primeira trajetória válida (com pelo menos 1 ponto) para centralizar a câmera do mapa
        center_coords = None
        for t in trajs:
            if t and len(t) > 0:
                center_coords = (t[0].y, t[0].x)
                break
                
        # Se não achou nenhum centro válido, devolve um mapa zerado
        ...
        for points in trajs:
            # TRAVA DE SEGURANÇA 2 (O SEU ERRO ESTAVA AQUI): 
            # Ignora a trajetória se ela tiver menos de 2 pontos (pois não forma uma linha)
